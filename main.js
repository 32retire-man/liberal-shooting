...
let teta=[];
let jiki = new Jiki();
// teki[0] = new Teki(6,200<<8, 200<<8, 0, 0)

//ファイルを読み込み
let ryoImage = new Image();
...
function drawAll() {
  //描画の処理
  vcon.fillStyle=(jiki.damage)?"#333":"black";
...
function putInfo() {
  ...
  y = CANVAS_H/2 - 20+20;
  con.fillText(s, x, y);
} else if(gameWave==2 && teki.length ==0) {
  s = "GAME CLEAR";
  w = con.measureText(s).width;
  x = CANVAS_W/2 - w/2;
  y = CANVAS_H/2 - 20;
...
　//テスト
  if(gameWave==0) {
    ...
    if(gameCount > 60*20) {
      ...
      gameWave++;
      gameCount=0;
      teki.push(new Teki(1,(FIELD_W/2)<<8,-(70<<8),0,200));
  } else if(gameWave==2) {
/*    if(teki.length == 0) {
      gameWave=0;
      gameCount=0;
...
window.onload=function() {
  gameInit();
//  teki.push(new Teki(1,(FIELD_W/2)<<8,0,0,200));
}
